...
<div class="row">
    ...
    <div class="col-md-4">
        <div class="card mb-4">
            ...
            <div class="card-body">
                <h4>Terkumpul Rp <span style="font-weight: bolder; color:#0d6efd">{{ number_format($totalDonasiProgram, 0, ',', '.') }}</span></h4>
                <h6 class="text-muted">dari target Rp <span>{{ number_format($program->target_dana, 0, ',', '.') }}</span></h6>
                <div class="progress mb-2">
                    <div class="progress-bar" role="progressbar" style="width: {{ $program->progress_percentage }}%;" aria-valuenow="{{ $program->progress_percentage }}" aria-valuemin="0" aria-valuemax="100">{{ $program->progress_percentage }}%</div>
                </div>
                <hr>
                <h4>Sisa Dana <span style="font-weight: bolder; color:#29b011">Rp {{ number_format($program->dana_terkumpul, 0, ',', '.') }}</span></h4>
